...
func GetMatchSummary(accountID, region string) []interface{} {
	matchHistory := GetMatchHistory(accountID, region, 0, 5)
	matchSummary := make([]interface{}, len(matchHistory.Matches))

	for i := 0; i < len(matchHistory.Matches); i++ {
		match := matchHistory.Matches[i]
		game := GetGameData(match.GameID, region).(map[string]interface{})
...
func GetGameData(matchID int64, region string) interface{} {
